...
export default class extends think.BaseSocket {
    ...
    async allEventReportAction() {
        const obj = this.post('userId,objName,ObjPosition,BGID,area_code,remarks,baidu_x,baidu_y,source,businesskey,group_code,parent_name,sub_name,type,img_push');
        obj.create_by = obj.userId || this.getUser().userId;
    ...
    async queryPartChildrenAction(){
        let data = this.post();
        try{
            let res = await this.service('cms/EventService').partChildren(data);
