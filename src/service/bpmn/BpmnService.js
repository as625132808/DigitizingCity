...
//解析流程元素来源，目标
    ...
    async  startProcessInstance(key,id,userID,data) {
        ...
        try {
            ...
            SendMsgObj.url=_this.pushServerUrl;
           await this.db.sequelize.transaction(async transaction=> {
                let transactionPromises = [];
                return _this.db['bpmn_hi_procinst'].create(procInstModel, {transaction:transaction}).then(async procInst=>{
                    const actinstModel=await _this.startActinst(proc[0],procModelArr,procInst);
                    const taskInstModel= await _this.startTask(proc[0],procModelArr,procInst);
                    SendMsgObj.date=moment(procInst.start_time).format("YYYY-MM-DD HH:mm:ss");
                    ...
                    transactionPromises.push(_this.db['bpmn_hi_actinst'].bulkCreate(actinstModel.arr,{transaction:transaction}))
...
//得到流程任务列表
    ...
    async startActinst(proc,arr,procInst){
      ...
      // const actinst=await this.commonService.saveMany(db['bpmn_hi_actinst'],arrs);
        return {userIds:userIds,arr:arrs};
    }
    ...
    async executeFlow(flowid, taskDescription, act_name, userid, proc_inst_id, proc_element_id,filePath,approveType) {
        ...
        try{
            ...
            SendMsgObj.result=act_name;
            await this.db.sequelize.transaction(async transaction=> {
                let transactionPromises = [];
                transactionPromises.push(_this.db['bpmn_hi_taskinst'].update( taskModel, {where:{id:taskArr[0].id},transaction}));
                transactionPromises.push(_this.db['bpmn_hi_actinst'].update( actModel, {where:{ proc_inst_id: proc_inst_id, proc_element_id: taskArr[0].dataValues.proc_element_id },transaction}));
                const taskInstModel = await _this.executeTask(userTaskAct[0].dataValues, procModelArr, procArr, flowid);
            ...
            return {SendMsgObj:SendMsgObj,userId:[createUserMoel.id],nextActinstIds:actinstModel.nextActinstIds,result:act_name,error:error};
        }catch(e){
            console.log(e)
        }
    ...
    async executeTask(proc,arr,procArr,flowid){
        ...
        return {arrs:arrs,id:proc.proc_inst_id,procModel:procModel};
    }
    //得到userTask往下执行的flow
   ...
   async processList(formType){
       ...
       try {
           ...
           const procinstSql = `select a.*,bpmn_re_deployment.is_hang
                       from bpmn_re_procdef a inner JOIN (select c.key ,max(c.rev) as rev ,c.deployment_id from bpmn_re_procdef c GROUP BY  c.key,c.deployment_id   ) b 
                       on a.key=b.key and a.rev=b.rev and a.deployment_id=b.deployment_id inner  JOIN bpmn_re_deployment on bpmn_re_deployment.id=a.deployment_id 
...
//流程列表流程保存
    ...
    async queryBpmnTypeList() {
        try {
            let sql = `SELECT	bpmn_re_procdef.deployment_id,	bpmn_re_procdef.KEY,	bpmn_re_procdef.NAME  FROM	bpmn_hi_procinst
                       left JOIN bpmn_re_procdef on bpmn_re_procdef.id=bpmn_hi_procinst.proc_def_id  where bpmn_re_procdef.KEY is not null
                        group by bpmn_re_procdef.key,bpmn_re_procdef.deployment_id`
