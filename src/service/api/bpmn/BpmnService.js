...
export default class  extends think.Service   {
    ...
    async getHomeFormList(user) {
        ...
        try {
            ...
            const ModelSql=`select  a.id,a.name as template_name,a.key as template_code,a.template_group,CONCAT('${this.serverIP}',a.icon_path) as template_icon from bpmn_re_procdef a INNER JOIN (select c.key ,max(c.rev) as rev ,c.deployment_id from bpmn_re_procdef c GROUP BY  c.key ,c.deployment_id  ) b 
                        on a.key=b.key and a.rev=b.rev and a.deployment_id=b.deployment_id INNER JOIN bpmn_re_deployment on bpmn_re_deployment.id=a.deployment_id and bpmn_re_deployment.is_hang=1
...
//待我审批，我已审批，我发起的审批
    ...
    async submitForm(userId,formId,data) {
        ...
        try {
            ...
            await this.db.sequelize.transaction(async transaction=> {
                ...
                return _this.db['bpmn_hi_procinst'].create(procInstModel, {transaction:transaction}).then(async procInst=>{
                    const actInstModel=await _this.startActinst(procdef[0],procModelArr,procInst);
                    const taskInstModel= await _this.startTask(procdef[0],procModelArr,procInst);
                    transactionPromises.push(_this.db['bpmn_hi_taskinst'].bulkCreate(taskInstModel, {transaction:transaction}))
                    transactionPromises.push(_this.db['bpmn_hi_actinst'].bulkCreate(actInstModel.arrs,{transaction:transaction}))
                    _this.pushDataToBacklogPeople(actInstModel.userIds,procdef[0].dataValues.form_type,data,procInstModel.review_name,procInst.dataValues.id);
                    result= _this.pushDataToWebBacklogPeople(actInstModel.userIds,procdef[0].dataValues.form_type,data,procInstModel.review_name,procInst.dataValues.id,userId);
    ...
    async startActinst(proc,arr,procInst){
       ...
       //const actinst=await this.commonService.saveMany(db['bpmn_hi_actinst'],arrs);
        ...
        return {userIds:userIds,arrs:arrs};
    }
    /**
     * 得到审批人
     * @returns
        ...
        try {
            ...
            for(let i in procArr){
                ...
                if(procArr[i].act_type=='UserTask'){
                    ...
                    }else if(procArr[i].depart!=''){
                            let departArr=JSON.parse(procArr[i].depart);
                            let orgArr=await this.db['sys_org'].findAll({where:{id:departArr}});
                            orgArr.forEach(item=>{
                                if(item.primary_person!=null){
                                    let arr=item.primary_person.split(',')
                                    arr.forEach(param=>{
                                        userIdArrs.push("'"+param+"'")
                                    })
                                }
                                if(item.deputy_person!=null){
                                    let arr=item.deputy_person.split(',')
    ...
    async submitFlowForm(flowId,option,userId,procInstId,uploadImgs,approveType) {
        ...
        try {
           ...
           //  const actUpdateResult = await _this.commonService.updateByWhere(this.db['bpmn_hi_actinst'], actModel, { proc_inst_id: procInstId, proc_element_id: taskArr[0].dataValues.proc_element_id });
            ...
            await this.db.sequelize.transaction(async transaction=> {
                let transactionPromises = [];
                transactionPromises.push(_this.db['bpmn_hi_taskinst'].update( taskModel, {where:{id:taskArr[0].id},transaction}));
                transactionPromises.push(_this.db['bpmn_hi_actinst'].update( actModel, {where:{ proc_inst_id: procInstId, proc_element_id: taskArr[0].dataValues.proc_element_id },transaction}));
                const taskInstModel = await _this.executeTask(userTaskAct[0].dataValues, procModelArr, procArr, flowId,approveType);
                actinstModel =await _this.executeActinst(userTaskAct[0].dataValues, procModelArr, procArr, flowId);
                transactionPromises.push(_this.db['bpmn_hi_actinst'].bulkCreate(actinstModel.arrs,{transaction:transaction}))
                if(taskInstModel.procModel!=null){
                    transactionPromises.push(_this.db['bpmn_hi_procinst'].update( taskInstModel.procModel, {where:{id:taskInstModel.id},transaction}));
                }
                transactionPromises.push(_this.db['bpmn_hi_taskinst'].bulkCreate(taskInstModel.arrs, {transaction:transaction}))

                return Promise.all(transactionPromises)
            }).then( async result=>{
                actinstModel.nextActinstIds=[];
                result[2].forEach(item=> {
                    if(item.act_type=='UserTask'&&item.assignee==''&&item.group==''){
                        actinstModel.nextActinstIds.push(item.id);
                    }
                })
                this.pushDataToBacklogPeople(actinstModel.userIds,procDef.form_type,procArr[0].form_json,procArr[0].review_name,userTaskAct[0].dataValues.proc_inst_id);
                this.pushDataToUserPeople(startUser.id,procDef.form_type,procArr[0].form_json,procArr[0].review_name,userTaskAct[0].dataValues.proc_inst_id,flowModel.act_name);
                 backObj= this.pushDataToWebBacklogPeople(actinstModel.userIds,procDef.form_type,procArr[0].form_json,procArr[0].review_name,userTaskAct[0].dataValues.proc_inst_id,userId);
                 createObj= this.pushDataToWebCreatePeople(startUser.id,procDef.form_type,procArr[0].form_json,procArr[0].review_name,userTaskAct[0].dataValues.proc_inst_id,userId,flowModel.act_name);
                // result 是事务回调中使用promise链中执行结果
                // Transaction 会自动提交
            }).catch(function (err) {
            ...
            return {backObj:backObj,createObj:createObj,nextActinstIds:actinstModel.nextActinstIds,error:error};
...
//执行历史任务
    async executeTask(proc,arr,procArr,flowid){
        ...
        return {procModel:procModel,id:proc.proc_inst_id,arrs:arrs};
    }
//得到userTask往下执行的flow
...
//得到往下执行的节点
  ...
  async  pushDataToUserPeople(backLogsUserIds,type,form,review_name,procInstId,result){
     ...
     * @returns
     */
    async updatePartState(procid) {
        try {
            let sql=`select oa_case.id,bpmn_hi_procinst.status from bpmn_hi_procinst left JOIN oa_case on oa_case.proc_inst_id=bpmn_hi_procinst.id
                where bpmn_hi_procinst.id=${procid}`
            let procCaseMsg=await this.commonService.querySql(sql);
            if(procCaseMsg[0].status!=1){
                return {};
            }
            let updateState = await this.commonService.updateById(this.db['cms_parts'],{ObjState:'完好'},procCaseMsg[0].id);
            return updateState
        } catch (error) {
            return this.exceptionService.handleError(error);
        }
